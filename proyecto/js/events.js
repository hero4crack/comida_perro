...
import { login, register, placeOrder, savePet, savePrecioConfig, loadReporte, calcularPrecio } from './api.js';
...
export function bindEvents() {
    ...
    async function actualizarPrecio() {
        ...
        if (sabor && periodo && gramos > 0) {
            ...
            if (montoTotal) montoTotal.value = precio.toFixed(2);
            if (precioCalculado) {
    ...
    if (addPetBtn) addPetBtn.addEventListener('click', () => { state.showPetForm = true; render(); });
    
    const cancelPetBtn = document.getElementById('btn-cancel-pet');
    if (cancelPetBtn) cancelPetBtn.addEventListener('click', () => { state.showPetForm = false; render(); });

    const addPrecioBtn = document.getElementById('btn-add-precio');
    if (addPrecioBtn) addPrecioBtn.addEventListener('click', () => { state.showPrecioForm = true; render(); });
    
    const cancelPrecioBtn = document.getElementById('btn-cancel-precio');
    if (cancelPrecioBtn) cancelPrecioBtn.addEventListener('click', () => { state.showPrecioForm = false; render(); });
    ...
    if (desdeEl) desdeEl.addEventListener('change', (e) => { state.desde = e.target.value; });
    if (hastaEl) hastaEl.addEventListener('change', (e) => { state.hasta = e.target.value; });
